...
import { getFirestore, Firestore, doc, getDoc, setDoc, onSnapshot, Unsubscribe, collection, query, getDocs, deleteDoc, updateDoc } from 'firebase/firestore';
...
export const getAllConfigs = async (): Promise<Array<UserConfig & { activationCode: string }>> => {
  try {
    const db = getFirestoreDb();
    ...
    console.log('[Firebase] Retrieved', configs.length, 'configurations');
    return configs;
  } catch (error) {
...
export const deleteConfigByCode = async (code: string): Promise<void> => {
  try {
    const db = getFirestoreDb();
    const configRef = doc(db, 'userConfigs', code);
    await deleteDoc(configRef);
    console.log('[Firebase] Config deleted for code:', code);
  } catch (error) {
...
export const toggleConfigActive = async (code: string, active: boolean): Promise<void> => {
  try {
    const db = getFirestoreDb();
    const configRef = doc(db, 'userConfigs', code);
    ...
    console.log('[Firebase] Config active status updated for code:', code, 'to:', active);
  } catch (error) {
    console.error('[Firebase] Error toggling config active:', error);
    throw error;
  }
};
